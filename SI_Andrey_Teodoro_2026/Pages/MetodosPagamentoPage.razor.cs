...
public partial class MetodosPagamentoPage : ComponentBase
{
    [Inject] private IMetodoPagamentoService MetodoPagamentoService { get; set; } = null!;
    [Inject] private ISnackbar Snackbar { get; set; } = null!;
    [Inject] private IDialogService DialogService { get; set; } = null!;

    private PaginacaoDto<MetodoPagamentoListDto>? _resultado;
    ...
    private async Task AlterarStatus(int id, string nome, bool ativoAtual)
    {
        var param = new DialogParameters<ConfirmDialog>
        {
            ...
            { x => x.Mensagem,   $"Deseja realmente {(ativoAtual ? "desativar" : "ativar")} o método \"{nome}\"?" },
        ...
        {
            var (sucesso, mensagem) = await MetodoPagamentoService.AlterarStatusAsync(id, !ativoAtual);
            Snackbar.Add(mensagem, sucesso ? Severity.Success : Severity.Error);
