...
public partial class CategoriasPage : BasePage<CategoriaListDto, CategoriaDto>
{
    ...
    {
        try { _carregando = true; _resultado = await CategoriaService.ObterTodosAsync(_filtro); }
    ...
    private Task AlterarStatus(int id, string nome, bool ativoAtual)
        => ConfirmarAlteracaoStatus(id, nome, ativoAtual, CategoriaService.AlterarStatusAsync, CarregarDados);
}