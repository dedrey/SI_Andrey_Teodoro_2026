...
public partial class CategoriasPage : ComponentBase
{
    [Inject] private ICategoriaService CategoriaService { get; set; } = null!;
    [Inject] private ISnackbar Snackbar { get; set; } = null!;
    [Inject] private IDialogService DialogService { get; set; } = null!;

    private PaginacaoDto<CategoriaListDto>? _resultado;
    ...
    private async Task AlterarStatus(int id, string nome, bool ativoAtual)
    {
        var param = new DialogParameters<ConfirmDialog>
        {
            ...
            { x => x.Mensagem,   $"Deseja realmente {(ativoAtual ? "desativar" : "ativar")} a categoria \"{nome}\"?" },
        ...
        {
            var (sucesso, mensagem) = await CategoriaService.AlterarStatusAsync(id, !ativoAtual);
            Snackbar.Add(mensagem, sucesso ? Severity.Success : Severity.Error);
