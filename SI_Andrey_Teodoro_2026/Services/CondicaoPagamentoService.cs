...
public class CondicaoPagamentoService : ICondicaoPagamentoService
{
    ...
    public async Task<(bool sucesso, string mensagem, int id)> SalvarAsync(CondicaoPagamentoDto dto)
    {
        ...
        catch (Exception ex) { return (false, $"Erro ao salvar condição de pagamento: {ex.Message}", 0); }
    }
    ...
    public async Task<(bool sucesso, string mensagem)> AlterarStatusAsync(int id, bool ativar)
    {
        try
        {
            ...
            return (true, $"Condição de pagamento {(ativar ? "ativada" : "desativada")} com sucesso!");
        }
