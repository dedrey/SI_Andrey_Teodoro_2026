...
public class MetodoPagamentoRepository : IMetodoPagamentoRepository
{
    ...
    public MetodoPagamentoRepository(DbConnectionFactory factory) => _factory = factory;
    ...
    public async Task<PaginacaoDto<MetodoPagamentoListDto>> ObterTodosAsync(FiltroConsultaDto filtro)
    {
        ...
        where.Add(filtro.StatusFiltro switch { "ativos" => "m.ativo = TRUE", "inativos" => "m.ativo = FALSE", _ => "1=1" });
        var whereClause = "WHERE " + string.Join(" AND ", where);
        var orderBy = filtro.OrdenarPor switch { "id" => "m.id", "data" => "m.criado_em", "codigo" => "m.codigo", _ => "m.metodo_pagamento" };
    ...
    public async Task<int> InserirAsync(MetodoPagamentoDto dto)
    {
        ...
        var proximoId = await conn.ExecuteScalarAsync<int>(
            @"SELECT MIN(seq)
              FROM (SELECT 1 AS seq UNION ALL SELECT id + 1 FROM metodos_pagamento) t
              WHERE seq NOT IN (SELECT id FROM metodos_pagamento)");
    ...
    public async Task AlterarStatusAsync(int id, bool ativo)
    {
        using var conn = _factory.CreateConnection();
        await conn.ExecuteAsync(
            "UPDATE metodos_pagamento SET ativo = @ativo, atualizado_em = NOW() WHERE id = @id",
            new { ativo, id });
    }
