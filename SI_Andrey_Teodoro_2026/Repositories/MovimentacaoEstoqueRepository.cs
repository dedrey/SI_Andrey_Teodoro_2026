...
public class MovimentacaoEstoqueRepository : BaseRepository, IMovimentacaoEstoqueRepository
{
    ...
    public async Task<PaginacaoDto<MovimentacaoEstoqueListDto>> ObterTodosAsync(FiltroConsultaDto filtro)
    {
        ...
        var orderBy = filtro.OrdenarPor switch { "tipo" => "m.tipo_movimentacao", "id" => "m.id", _ => "m.criado_em DESC" };
        var param = new { Busca = $"%{filtro.Busca}%", BuscaExata = filtro.Busca, StatusFiltro = filtro.StatusFiltro, Limit = filtro.TamanhoPagina, Offset = (filtro.Pagina - 1) * filtro.TamanhoPagina };
        var total = await conn.ExecuteScalarAsync<int>($"SELECT COUNT(*) FROM movimentacoes_estoque m {whereClause}", param);
        var itens = await conn.QueryAsync<MovimentacaoEstoqueListDto>(
            $@"SELECT m.id, m.tipo_movimentacao AS TipoMovimentacao, m.observacao,
                      ...
                      m.criado_em AS CriadoEm, u.nome AS NomeCriadoPor
        ...
        return new PaginacaoDto<MovimentacaoEstoqueListDto> { Itens = itens.ToList(), TotalItens = total, Pagina = filtro.Pagina, TamanhoPagina = filtro.TamanhoPagina };
    }
    ...
    public async Task<MovimentacaoEstoque?> ObterPorIdAsync(int id)
    {
        ...
        return await conn.QueryFirstOrDefaultAsync<MovimentacaoEstoque>(
            @"SELECT m.id, m.tipo_movimentacao AS TipoMovimentacao, m.observacao,
                     m.criado_em AS CriadoEm, m.criado_por AS CriadoPor, u.nome AS NomeCriadoPor
              FROM movimentacoes_estoque m
              LEFT JOIN usuarios u ON u.id = m.criado_por
    ...
    public async Task<List<MovimentacaoEstoqueItemListDto>> ObterItensPorMovimentacaoAsync(int movimentacaoId)
    {
        ...
        var result = await conn.QueryAsync<MovimentacaoEstoqueItemListDto>(
            @"SELECT i.id, i.movimentacao_id AS MovimentacaoId, i.produto_variacao_id AS ProdutoVariacaoId,
    ...
    public async Task<int> InserirAsync(MovimentacaoEstoqueDto dto)
    {
        ...
        await conn.ExecuteAsync(
            "INSERT INTO movimentacoes_estoque (id, tipo_movimentacao, observacao) VALUES (@ProximoId, @TipoMovimentacao, @Observacao)",
            new { ProximoId = proximoId, dto.TipoMovimentacao, dto.Observacao });
        return proximoId;
    }
