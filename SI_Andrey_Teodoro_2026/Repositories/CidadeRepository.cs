...
public class CidadeRepository : ICidadeRepository
{
    ...
    public CidadeRepository(DbConnectionFactory factory)
        => _factory = factory;
    ...
    {
        using var conn = _factory.CreateConnection();
        var sql = @"SELECT c.id, c.cidade AS NomeCidade, c.ddd, c.estado_id AS EstadoId,
                           e.estado AS NomeEstado, e.uf, p.pais AS NomePais,
                           c.ativo, c.criado_em AS CriadoEm
                    FROM cidades c
                    INNER JOIN estados e ON e.id = c.estado_id
                    INNER JOIN paises  p ON p.id = e.pais_id
                    WHERE c.id = @id";
        return await conn.QueryFirstOrDefaultAsync<Cidade>(sql, new { id });
    }

    public async Task<int> InserirAsync(CidadeDto dto)
    {
        using var conn = _factory.CreateConnection();
        var sql = @"INSERT INTO cidades (cidade, ddd, estado_id, ativo)
